...
export const allFalseReducers = {
    projectStart: (
        { list = R('list'), node = R('node') } = R('properties')
    ) => ({
        list,
        currentProject: node,
        currentGroup: false
...
export const currentGroupOnlyReducers = {
    ...
    projectTerminal: (
        { list = R(' list'), currentGroup = R('currentGroup ') } = R(
            'properties'
        )
    ) => ({
        list: list.concat(currentGroup),
 ...
 * @returns {Parent}
 */
export const updateParentWithNode = (
    parent = { childNodes: [] },
    childNode
) => ({
    ...parent,
...
const constructMatcher = reducers => node =>
    match(
        _.reduce(
            ...
            (acc, v, k) => ({
                ...acc,
                [k]: () => v // match automatically calls a function, so this will wrap the reducer so it is not called before it can be passed the payload
            }),
            {}
        ),
